...
public:
	...
	void BindParem(int32 idx, T(&value)[N])
	{
	...
	template<typename T>
	void BindParem(int32 idx, T* value, int32 N)
	{
		_dbConnection.BindParam(idx + 1, (const BYTE*)value, size32(T) * N, &_paramIndex[idx]);
		_paramFlag |= (1LL << idx);
	}
	...
	void BindCol(int32 idx, const WCHAR* value ,int32 len)
	{
	...
	{
		_dbConnection.BindCol(idx + 1, (const BYTE*)value, size32(T) * N, &_columnIndex[idx]);
		_columnFlag |= (1LL << idx);
	}

	template<typename T>
	void BindCol(int32 idx, T* value, int32 N)
	{
		_dbConnection.BindCol(idx + 1, (const BYTE*)value, size32(T) * N, &_columnIndex[idx]);
		_columnFlag |= (1LL << idx);
	}
